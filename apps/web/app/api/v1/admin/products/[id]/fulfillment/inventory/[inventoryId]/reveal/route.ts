...
async function pathFor(context: Context, action: "reveal" | "revoke"): Promise<string | Response> {
  ...
  return `/api/v1/admin/products/${productId}/fulfillment/inventory/${inventoryId}/${action}`;
}

export async function POST(request: Request, context: Context) {
  const path = await pathFor(context, "reveal");
