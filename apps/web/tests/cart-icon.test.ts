...
describe("Icons8 shopping cart asset", () => {
  it("ships the requested cart icon locally", () => {
    ...
    expect(existsSync(resolve(webRoot, "public/icons/icons8-cart.gif"))).toBe(false);
  });
  ...
  it("uses the local cart icon instead of an emoji or remote runtime URL", () => {
    ...
    expect(css).not.toContain('url("/icons/icons8-cart.gif")');
    expect(css).not.toContain(".cart-trigger:hover .cart-icon");
    expect(css).not.toContain(".cart-button:not(:disabled):hover .cart-icon");
