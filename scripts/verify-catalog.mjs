...
// Tuple: visual code, exact English name, source USD, type, shown stock/bundle count.
const expectedCatalog = [
  ["CL", "Creator Launch Kit", 29, "BUNDLE", 4],
  ["UI", "Aurora UI Component Library", 34, "SINGLE", 88],
  ["SP", "Social Post Template Collection", 18, "BUNDLE", 6],
  ["BG", "Minimal Brand Guidelines", 24, "SINGLE", 70],
  ["PD", "Editorial Presentation Deck", 16, "BUNDLE", 3],
  ["IQ", "Invoice & Quote Template Bundle", 9, "SINGLE", 200],
  ["FP", "Freelance Project Starter Pack", 14, "BUNDLE", 5],
  ["PC", "Podcast Cover Art Templates", 12, "SINGLE", 82],
  ["MT", "Motion Title Graphics Pack", 28, "BUNDLE", 4],
  ["CP", "Cinematic Color Presets", 19, "SINGLE", 140],
  ["3D", "Abstract 3D Shapes Collection", 22, "BUNDLE", 3],
  ["IC", "Essential Interface Icon Set", 15, "SINGLE", 180],
  ["CV", "Modern Resume & Portfolio Kit", 13, "BUNDLE", 4],
  ["DF", "Daily Focus Planner", 7, "SINGLE", 230],
  ["TJ", "Travel Journal Page Bundle", 8, "BUNDLE", 3],
  ["RB", "Recipe Book Layout Template", 18, "SINGLE", 76],
  ["BS", "Personal Budget Spreadsheet", 11, "SINGLE", 210],
  ["FD", "Small Business Finance Dashboard", 27, "BUNDLE", 2],
  ["PW", "Project Planning Workspace", 21, "SINGLE", 99],
  ["CW", "Online Course Workbook", 17, "BUNDLE", 3],
  ["WW", "Website Wireframe Library", 32, "SINGLE", 49],
  ["EM", "Email Campaign Template Set", 14, "BUNDLE", 5],
  ["VT", "Video Thumbnail Design Pack", 12, "SINGLE", 160],
  ["SF", "Sound Effects Starter Library", 23, "BUNDLE", 4],
  ["AL", "Ambient Audio Loop Collection", 16, "SINGLE", 58],
  ["PH", "Lifestyle Stock Photo Bundle", 36, "BUNDLE", 6],
  ["HF", "Handwritten Font Pair", 19, "SINGLE", 125],
  ["EF", "Editorial Serif Display Font", 25, "BUNDLE", 2],
  ["MI", "Monoline Illustration Kit", 20, "SINGLE", 77],
  ["GB", "Gradient Background Collection", 10, "BUNDLE", 8],
  ["PM", "E-commerce Product Mockups", 31, "SINGLE", 52],
  ["PK", "Packaging Mockup Essentials", 26, "BUNDLE", 4],
  ["MA", "Mobile App Showcase Scenes", 24, "SINGLE", 80],
  ["BC", "Creative Business Card Pack", 9, "BUNDLE", 5],
  ["NL", "Newsletter Layout System", 18, "SINGLE", 90],
  ["LC", "Digital Product Launch Checklist", 6, "SINGLE", 240],
];
...
assert.deepEqual(catalog.priceRange, {
  minMinor: Math.min(...expectedCatalog.map((entry) => entry[2] * 35 * 100)),
  maxMinor: Math.max(...expectedCatalog.map((entry) => entry[2] * 35 * 100)),
  currency: "THB",
});

for (const [index, expected] of expectedCatalog.entries()) {
  const [visualCode, nameEn, usd, type, shownCount] = expected;
  ...
  assert.equal(item.visualCode, visualCode, `Visual code mismatch at ${index + 1}`);
  assert.equal(item.nameEn, nameEn, `English name mismatch at ${index + 1}`);
  assert.equal(item.type, type, `Product type mismatch at ${index + 1}`);
  ...
  if (type === "BUNDLE") {
    assert.equal(item.bundleItemCount, shownCount, `Bundle count mismatch at ${index + 1}`);
    assert.equal(item.stockQuantity, 1, `Bundle availability sentinel mismatch at ${index + 1}`);
  } else {
    assert.equal(item.bundleItemCount, null, `Single item has bundle count at ${index + 1}`);
...
assert.equal(aurora.items[0]?.visualCode, "UI");
...
assert.equal(lowestPrice.items[0]?.visualCode, "LC");
